...
config_export() {
    ...
    content="# DeePloy config — generated by 'deeploy export' ($(date -u '+%Y-%m-%d %H:%M:%S')Z)
# Created with DeePloy v${ver}
# PATHS, PUBKEYS, and SETTINGS only — NEVER key material. chmod 600.
# Reuse on an identical box:  deeploy.sh install --config <this file>
"
    while read -r a b; do
        [[ -z "$a" ]] && continue
        if [[ "$a" == "@" ]]; then content+=$'\n'"# --- ${b} ---"$'\n'; continue; fi
