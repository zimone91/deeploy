...
# N15: fail2ban ships with its sshd jail on 'port ssh' (22). After the port
# change it would watch/ban the WRONG port — render a DROP-IN pinned to the
# port(s) sshd actually listens on, in the same phase that realizes the port.
# Explicit even on the default 22. fail2ban itself comes from BASE_PACKAGES.
# A jail.d/*.local drop-in (read LAST: jail.conf -> jail.d/*.conf -> jail.local
# -> jail.d/*.local) — NOT jail.local itself, which an operator may own:
# wholesale-rewriting jail.local would silently drop their ignoreip/custom
# jails from the live config. This file is DeePloy's alone.
_base_fail2ban_jail() {                           # <port[,port...]>
    local ports=$1 f="${FAIL2BAN_JAIL_DROPIN:-/etc/fail2ban/jail.d/deeploy-sshd.local}"
    write_file "$f" \
"# Generated by DeePloy — keep the fail2ban sshd jail on the REAL SSH port.
[sshd]
enabled = true
port = ${ports}
"
    run systemctl reload-or-restart fail2ban 2>/dev/null || true
}
...
base_firewall() {
    ...
    # fail2ban's sshd jail follows the realized port(s) — same phase, same source
    # of truth (comma-joined when multiple were confirmed above). (N15)
    _base_fail2ban_jail "$(printf '%s' "$ports" | tr '\n' ',')"
